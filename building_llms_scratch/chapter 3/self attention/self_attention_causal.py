...
class selfAttentionCausal(nn.Module):
    def __init__(self, dim_in, dim_out,seq_len, qwv_bias=False):
        super(selfAttentionCausal, self).__init__()
        self.dim_in = dim_in
    ...
    def forward(self, x):
        ...
        attn_scores = attn_scores.masked_fill(self.mask[:, :seq_length, :seq_length] == 0, float('-inf')) # B
